...
class ChatSystem(System):
    ...
    def init(self, *args, **kwargs) -> None:
        ...
        self.searchers = [
            Searcher(config_path=cfg, **self.agent_kwargs)
            for cfg in self.config['searchers']  # Lista de configurações dos searchers
        ]
        self.search_manager = SearchManager(searcher=self.searchers,config=self.config, **self.agent_kwargs)
    ...
    def execute(self, action_type: str, argument: Any):
        ...
        #     log_head = f':violet[Response from] :red[Searcher] :violet[with] :blue[{argument}]:violet[:]\n- '
            ...
            self.log(f":violet[Calling all Searchers with] :blue[{argument}]...", agent=self.manager, logging=False)
            # Usando o SearchManager para invocar os searchers
            results = self.search_manager.invoke(argument)
            observation = f"Negotiated results:\n- {results}"
            log_head = f":violet[Negotiated response]:\n- "
        else:
